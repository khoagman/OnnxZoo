...
public static class PostProcessing {
    ...
    public static TrackObject[] TrackObjectConvert(Prediction[] predictions) {
        var result = new List<TrackObject>();

        foreach (var prediction in predictions) {
            int[] xywh = Utils.Xyxy2xywh(new int[] 
            {   prediction.Coordinate[0], 
                prediction.Coordinate[1],
                prediction.Coordinate[2],
                prediction.Coordinate[3] 
            });

            float[] xywhFloat = new float[]
            {   xywh[0],
                xywh[1],
                xywh[2],
                xywh[3]
            ...
            result.Add(new TrackObject(xywhFloat, prediction.Label.Id, prediction.Score, prediction.Label.Name));
        }
